...
def eval_attack_hmean(
    ...
    assert utils.is_type_list(clean_results, dict)
    assert utils.is_type_list(img_infos, dict)
    ...
    assert 0 <= min_score_thr <= max_score_thr <= 1
    assert 0 <= step <= 1
    assert len(clean_results) == len(img_infos)
    assert isinstance(metrics, set)
    ...
    img_count = len(clean_results)
    gts_ignore = [[] for i in range(img_count)]
    gts = []

    preds = []
    pred_scores = []

    #get attacked results

...
from mmocr.apis.inference import model_inference, model_attack
...
class MMOCR:
    ...
    def __init__(self,
                 ...
                 kie_config='',
                 kie_ckpt='',
                 config_dir=os.path.join(str(Path.cwd()), 'detlib/mmocr/configs/'),
                 device=None,
                 **kwargs):
    ...
    def attack_text(self,
        ...
        if self.detect_model and self.recog_model:
            ...
            for model in list(
                    filter(None, [self.recog_model, self.detect_model])):
                result = self.single_attack(model, args.arrays,
                                               args.batch_mode,
                                               args.single_batch_size,
                                               perturbation)
                #pp_result = self.single_pp(result, model)
    ...
    def single_pp(self, result, model):
        ...
        # Post processing function for separate det/recog inference
    def single_pp_attack(self, result, model):
        for arr, output, export, res in zip(self.args.arrays, self.args.output,
                                            self.args.export, result):
            if export:
                mmcv.dump(res, export, indent=4)
    ...
    def single_attack(self, model, arrays, batch_mode, batch_size=0, perturbation=None):
        def inference(m, a, **kwargs):
            if model == 'Tesseract_det':
                return self.tesseract_det_inference(a)
            elif model == 'Tesseract_recog':
                ...
                return model_attack(m, a, perturbation, **kwargs)
        result = []
        if batch_mode:
            if batch_size == 0:
                result = inference(model, arrays, batch_mode=True)
            ...
            # return inference(model, arr, batch_mode=False)

...
def parse_args(arg_list=None):
    ...
    parser.add_argument(
        '--perturbation',
        default='/home/yexiaoyu/mmocr_attack/text.pth',
        help='perturbation filename to be evaluated.')
...
def run_test_cmd(args):
    ...
    print("Evaluating with perturbation file: {}".format(args.perturbation))
    pertubation = torch.load(args.perturbation,map_location='cpu')

    if not distributed:
        model = MMDataParallel(model, device_ids=cfg.gpu_ids)
        is_kie = cfg.model.type in ['SDMGR']
        clean_outputs,adv_outputs = single_gpu_attack_test(model, data_loader, pertubation, args.show, args.show_dir,
    ...
    if rank == 0:
        if args.out:
            ...
            mmcv.dump(adv_outputs, args.out)
        kwargs = {} if args.eval_options is None else args.eval_options
        if args.format_only:
            dataset.format_results(adv_outputs, **kwargs)
        if args.eval:
            ...
            print(results)
            #with open(file="/home/yexiaoyu/mmocr_attack/eval_compare_results.txt",encoding="utf-8") as f:
            #    f.write(f"{results['0_hmean-iou:recall']}\t{results['0_hmean-iou:precision']}\t{results['0_hmean-iou:hmean']}\n")
